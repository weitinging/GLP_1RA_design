...
def bfactor(OF_pdb):
    ...
    myspace_all = {'bfactors': []}
    cmd.iterate(name, 'bfactors.append(b)', space=myspace_all)
    all_plddt = np.mean(myspace_all['bfactors'])
    myspace_chain = {'bfactors': []}
    cmd.iterate("chain A", 'bfactors.append(b)', space=myspace_chain)
    chain_plddt = np.mean(myspace_chain['bfactors'])
    cmd.delete('all')
    return all_plddt, chain_plddt
    ...
    with open(out_file, 'w') as out:
        ...
        for pdb_file in sorted(os.listdir(design_path), key=natural_sort_key):
            if pdb_file.endswith(".pdb"):
                ...
                pLDDT, chain_plddt = bfactor(des_pdb)
                out.write('%s\t%.3f\t%.3f\t%.3f\n' % (pdb, rmsd, pLDDT, chain_plddt))
                logger.info(f"Sample: {pdb}, motif-RMSD = {rmsd}, pLDDT = {pLDDT}, chain_pLDDT = {chain_plddt}")
